...
namespace Prueba_extensiones.App
{
	public partial class Form1 : Form
	{
		...
		private void LoadFilters(XmlConfiguration configuration)
		{
			...
			if (configuration.TreatedAsRoot == null)
				configuration.TreatedAsRoot = new List<ElementNameFilter>();
			if (configuration.IgnoredClasses == null)
		...
		private void btnListAdd_Click(object sender, EventArgs e)
		{
			...
			if (dialog != null && listBox != null)
			{
				...
				{
					listBox.Items.Add($"{dialog.Result.Namespace.ToString()}:{dialog.Result.Prefix.ToString()}:{dialog.Result.Name.ToString()}");
				}
			}
		}
		...
		private void Form1_FormClosing(object sender, FormClosingEventArgs e)
		{
			if (DialogResult == DialogResult.OK)
				Custom = (XmlConfiguration)Current.Clone();
			else
				Current = (XmlConfiguration)Custom.Clone();
		}
	}
